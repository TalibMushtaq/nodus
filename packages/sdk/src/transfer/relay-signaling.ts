...
// Path B signaling over the Relay WebSocket (shared by web and native).
//
// The client sends `webrtc_offer` and trickle ICE to the node, and the node's
// `webrtc_answer`/ICE come back on the same socket. The Relay forwards these
// envelopes without inspecting them (it never sees file bytes).
//
...
export interface RelayChannelDeps {
  /** Send an envelope; the Relay routes it by peer id. */
  send: (msg: { type: string; payload: unknown }) => void;
  /** Subscribe to inbound envelopes by message type; returns an unsubscribe fn. */
...
export function createSignedRelayChannel(deps: RelayChannelDeps): SignalingChannel {
