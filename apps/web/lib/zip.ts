...
//
// The browser has no standard "create a ZIP" API — `CompressionStream` only
// produces a raw gzip/deflate stream, not the ZIP container with its central
// directory — and the project carries no zip dependency. A STORE-only writer is
// small and correct, and the loss is minor: the dominant use is bundling a
// folder of already-encrypted Nodus files, whose bytes are high-entropy and
// would not compress meaningfully anyway.
