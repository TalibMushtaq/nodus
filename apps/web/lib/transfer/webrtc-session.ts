...
// Persistent WebRTC session for shard uploads.
//
// The previous per-shard flow negotiated a brand-new peer connection for every
// 8 MiB shard, so an 800 MB file paid ~100 SDP/ICE round trips before moving any
// bytes. A session negotiates once and then streams every shard of a transfer
// over the same data channel. The node side already supports this: its channel
// handler resets per-shard state after `shard_done` and keeps the session warm.
//
